...
const Orders = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE}/api/adverts/entrepreneur${query}`, {
          method: 'POST',
          headers,
        ...
        if (fetchError.name !== 'AbortError') {
          console.error('Error fetching entrepreneur adverts', fetchError);
  ...
  const getTypeBadgeLabel = useMemo(() => {
    return (item) => {
  ...
  const handleApplyFilters = () => {
    ...
    if (legalStatus) filters.legal_status = legalStatus;
  ...
  const renderCard = (item) => {
    ...
    return (
      <Link
        key={item.slug}
        to={`/sifarisler/${item.slug}`}
        state={{ ownerSlug: item?.user?.slug }}
