...
const typeDisplayLabels = {
  az: {
    individual: 'Fiziki şəxs elanları',
    legal_entity: 'Hüquqi şəxs elanları',
    entrepreneur: 'Sahibkar elanları',
    transporter: 'Daşıyıcı elanları'
  },
  en: {
    individual: 'Individual adverts',
    legal_entity: 'Legal entity adverts',
    entrepreneur: 'Entrepreneur adverts',
    transporter: 'Transporter adverts'
  },
  ru: {
    individual: 'Объявления физ. лиц',
    legal_entity: 'Объявления юр. лиц',
    entrepreneur: 'Объявления предпринимателей',
    transporter: 'Объявления перевозчиков'
  }
};

const userDirectoryPath = {
  individual: '/sexsler/fiziki-sexsler',
  legal_entity: '/sexsler/huquqi-sexsler',
  entrepreneur: '/sexsler/sahibkarlar',
  transporter: '/sexsler/sahibkarlar'
};
...
const AdvertDetail = () => {
  ...
  const [resolvedType, setResolvedType] = useState(
    typeParam || location.state?.advertType || null
  );
  ...
  const displayLabels = useMemo(
    () => typeDisplayLabels[language] || typeDisplayLabels.az,
    [language]
  );

  useEffect(() => {
    setResolvedType(typeParam || location.state?.advertType || null);
  }, [slug, typeParam, location.state?.advertType]);
  ...
  useEffect(() => {
    ...
    const fetchGenericAdvert = async (endpoint) => {
      try {
    ...
    const fetchEntrepreneurAdvert = async () => {
      const ownerSlug = location.state?.ownerSlug;
      if (ownerSlug) {
        const owner = await fetchOwnerDetails(ownerSlug);
        const match = owner?.adverts?.find((a) => a.slug === slug);
        if (match) {
          match.user = {
            name: owner.name,
      ...
      const advert = await fetchGenericAdvert(`${API_BASE}/api/adverts/entrepreneur/${slug}`);
      if (advert && !advert.user?.slug) {
    ...
    const fetchByType = async (type) => {
      switch (type) {
        case 'individual':
          return fetchGenericAdvert(`${API_BASE}/api/adverts/individuals/${slug}`);
        case 'legal_entity':
        case 'legal':
          return fetchGenericAdvert(`${API_BASE}/api/adverts/legal-entities/${slug}`);
        case 'entrepreneur':
        case 'transporter':
          return fetchEntrepreneurAdvert();
        default:
          return null;
      }
    };

    const resolveAdvert = async () => {
      if (!slug) return;
      if (!isActive) return;
      setLoading(true);
      setError(null);
      ...
      const baseOrder = ['individual', 'legal_entity', 'entrepreneur'];
      const initialType = typeParam || location.state?.advertType || null;
      const attemptOrder = initialType
        ? [initialType, ...baseOrder.filter((t) => t !== initialType)]
        : baseOrder;

      for (const type of attemptOrder) {
        const advert = await fetchByType(type);
        if (!isActive) return;
        if (advert && advert.slug) {
          setData(advert);
          setResolvedType(type === 'legal' ? 'legal_entity' : type);
          setSelectedImage(0);
          setZoom(1);
          setPan({ x: 0, y: 0 });
  ...
  }, [slug, typeParam, location.state?.advertType, token]);
  ...
  const handleNavigateUser = () => {
    if (!data?.user?.slug || !resolvedType) return;
    const basePath = userDirectoryPath[resolvedType] || '/sexsler/fiziki-sexsler';
    navigate(`${basePath}/${data.user.slug}`);
  };
  ...
  const typeHeader = resolvedType ? displayLabels[resolvedType] : '';
