          ...
          {isFullscreen && (
            <div className="fixed inset-0 z-[1001] bg-black/90 flex items-center justify-center" onMouseUp={() => setIsPanning(false)} onMouseLeave={() => setIsPanning(false)}>
              <div className="absolute top-4 right-4 flex gap-2">
                <button className="px-3 py-2 rounded bg-white/90 text-[#2E92A0]" onClick={() => setZoom((z) => Math.min(5, z + 0.25))}>+
                </button>
                <button className="px-3 py-2 rounded bg-white/90 text-[#2E92A0]" onClick={() => setZoom((z) => Math.max(1, z - 0.25))}>-
                </button>
