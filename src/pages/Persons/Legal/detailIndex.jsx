...
const LegalDetailIndex = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const res = await axios.get(`https://atfplatform.tw1.ru/api/company-details/${slug}`);
  ...
  return (
    <div className="w-full flex justify-center">
      <div className="w-full max-w-[2136px] px-[16px] md:px-[32px] lg:px-[50px] xl:px-[108px] py-6">
        ...
        {branches.length > 0 && (
          <div className="mt-6">
            <div className="text-lg font-medium text-[#3F3F3F] mb-3">Filiallar</div>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {branches.map((b, i) => (
                <div key={b.slug || i} className="bg-white border border-[#E7E7E7] rounded-lg overflow-hidden">
                  ...
                  <div className="p-4">
                    <div className="font-medium text-[#3F3F3F]">{b.name?.az}</div>
                    <div className="text-[#6B7280] text-sm">{b.description?.az}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {adverts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {adverts.map((a) => {
                ...
                return (
                  <a
                    href={`/dasinma/elanlar/legal_entity/${a.slug}`}
                    key={a.id}
...
export default LegalDetailIndex
