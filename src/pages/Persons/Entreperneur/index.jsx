...
const EntreperneurIndex = () => {
  ...
  const fetchEntrepreneurs = async (page = 1) => {
    try {
      setLoading(true);
      const res = await axios.get(`https://atfplatform.tw1.ru/api/entrepreneurs?page=${page}`);
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= pagination.lastPage) fetchEntrepreneurs(page);
  };

  useEffect(() => { fetchEntrepreneurs(1); }, []);
...
export default EntreperneurIndex